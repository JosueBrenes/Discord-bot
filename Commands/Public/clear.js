  ...
  async execute(interaction) {
    ...
    if (quantity > 100) {
      const limitExceededEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "You cannot delete more than 100 messages at once due to Discord API limitations.",
        )
        .setTimestamp();
      return interaction.reply({
        embeds: [limitExceededEmbed],
        ephemeral: true,
      });
    }
    ...
    if (quantity <= 0) {
      const invalidQuantityEmbed = new EmbedBuilder()
        ...
        .setDescription(
          "Please provide a valid number of messages to delete (greater than 0).",
        )
        .setTimestamp();
      return interaction.reply({
        embeds: [invalidQuantityEmbed],
        ephemeral: true,
      });
    }
