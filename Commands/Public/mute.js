...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("mute")
    .setDescription(
      "Mute a user for a specified time (isolating them from text and voice channels)",
    )
    ...
    .addStringOption((option) =>
      option
        .setName("time")
        .setDescription(
          "The time duration to mute (e.g., 10m for 10 minutes, 2h for 2 hours)",
        )
        .setRequired(true),
    )
    .addStringOption((option) =>
  ...
  async execute(interaction) {
    ...
    if (!timeMatch) {
      const embedInvalidTime = new EmbedBuilder()
        ...
        .setDescription(
          "Invalid time format. Use the format like `10m` for 10 minutes or `2h` for 2 hours.",
        )
    ...
    if (!muteIsolatedRole) {
      const embedRoleNotFound = new EmbedBuilder()
        ...
        .setDescription(
          'The "Muted and Isolated" role could not be found. Please ensure it exists.',
        )
        .setTimestamp();

      return interaction.reply({
        embeds: [embedRoleNotFound],
        ephemeral: true,
      });
    }
    ...
    setTimeout(async () => {
      ...
      const embedUnmute = new EmbedBuilder()
        ...
        .setDescription(
          `**User:** ${user.tag} is no longer isolated from text and voice channels.\n\n**Duration:** ${time}`,
        )
