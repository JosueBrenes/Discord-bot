    ...
    message.mentions.users.forEach((user) => {
      ...
      if (member && member.roles.cache.has(roleIdAdmin)) {
        const embedAlert = new MessageEmbed()
          ...
          .setTitle("Attention!")
          .setDescription(
            `${member.displayName} has a role that prevents you from bothering them.`,
          );
      ...
      if (mentionCount[user.id] === mentionThreshold) {
        const embedAdmin = new MessageEmbed()
          ...
          .setDescription(
            `User ${user.tag} has reached the mention limit of ${mentionThreshold}.`,
          );

        const adminMembers = message.guild.members.cache.filter((member) =>
          member.roles.cache.has(roleIdAdmin),
        );
    ...
    message.reply({
      embeds: [
        new MessageEmbed()
          ...
          .setDescription(
            "Yes, it's me **Isidoro God**. Happy to help you! Type `/help` or `t-help` to see the available commands and the various ways I can assist you. Also, remember to vote for the bot by clicking the vote button.",
          )
