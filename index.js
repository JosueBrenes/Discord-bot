...
function detectSpam(message) {
  ...
  const recentMessages = history.filter(
    (msg) => now - msg.timestamp < SPAM_TIME_LIMIT,
  );
  ...
  const hasRepeatedWords = Object.values(wordCounts).some(
    (count) => count >= SPAM_THRESHOLD,
  );
  if (hasRepeatedWords) {
    message.delete().catch((error) => {
      if (error.code === 10008) {
        console.log(
          "Attempted to delete an unknown or already deleted message.",
        );
      } else {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "Your message was deleted for repeatedly sending the same word in a single message.",
      )
      .setTimestamp();
  ...
  const contentWithoutMentionsForSymbols = content
    .replace(/<@!\d+>/g, "")
    .replace(/<@&\d+>/g, "")
    .split("")
    .filter((char) => /[\p{Emoji}\p{Symbol}]/u.test(char)).length;
  ...
  if (contentWithoutMentionsForSymbols > SYMBOL_THRESHOLD) {
    if (!content.includes("<@!") && !content.includes("<@&")) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle("⚠️ Spam Warning ⚠️")
        .setDescription(
          "Your message was deleted for containing too many symbols or emojis.",
        )
        .setTimestamp();
  ...
  const contentWithoutMentionsForDuplicates = content
    .replace(/<@!\d+>/g, "")
    .replace(/<@&\d+>/g, "");
  const duplicateMessages = recentMessages.filter(
    (msg) =>
      msg.content.replace(/<@!\d+>/g, "").toLowerCase() ===
      contentWithoutMentionsForDuplicates.toLowerCase(),
  );

  if (duplicateMessages.length > DUPLICATE_MESSAGE_LIMIT) {
    duplicateMessages.slice(1).forEach((dupMsg) => {
      channel.messages
        .fetch({ limit: MESSAGE_HISTORY_LIMIT })
        .then((messages) => {
          const msgToDelete = messages.find(
            (m) =>
              m.content === dupMsg.content &&
              m.author.id === author.id &&
              now - m.createdTimestamp < SPAM_TIME_LIMIT,
          );
          if (msgToDelete) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle("⚠️ Spam Warning ⚠️")
      .setDescription(
        "Your message was deleted for repeatedly sending duplicate messages.",
      )
      .setTimestamp();
